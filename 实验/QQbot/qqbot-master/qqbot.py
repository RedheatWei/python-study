...
########################################################################################
        for i in self.groups:
            if i['name'] == groupname:
                groupuid = i['uin']
        # for j in self.buddies:
        #     if j['qq'] == '527276470':
        #         qqid = j['uin']
        livestatus = livemod.LiveModelFunc(room_id)
        while not self.stopped:
            try:
                while True:
                    if livestatus.checkLiveStartJson() is False:
                        time.sleep(60*2)
                    else:
                        self.send('group', groupuid, sendmsg)
                        # self.send('buddy', qqid, sendmsg)
